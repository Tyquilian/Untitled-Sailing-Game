...
namespace WavePrototype.Editor
{
    public static partial class BatchBuild
    {
        ...
        [MenuItem("Wave Prototype/Build Batch 19 Windows")]
        public static void BuildBatch19()
        {
            try
            {
                RunValidation();
                EnsureScene();
                string output = Path.GetFullPath("Builds/Batch19/TacticalSailingBatch19.exe");
                Directory.CreateDirectory(Path.GetDirectoryName(output));
                var options = new BuildPlayerOptions
                {
                    scenes = new[] { ScenePath },
                ...
                if (report.summary.result != UnityEditor.Build.Reporting.BuildResult.Succeeded)
                    throw new InvalidOperationException("Windows build failed: " + report.summary.result);
                Debug.Log($"[WAVE-BUILD] SUCCESS batch=19: {output} ({report.summary.totalSize:N0} bytes, {report.summary.totalTime})");
            }
            catch (Exception exception)
            {
                Debug.LogException(exception);
        ...
        [MenuItem("Wave Prototype/Build Batch 20 Windows")]
        public static void BuildBatch20()
        {
            try
            {
                RunValidation();
                EnsureScene();
                string output = Path.GetFullPath("Builds/Batch20/TacticalSailingBatch20.exe");
                Directory.CreateDirectory(Path.GetDirectoryName(output));
                var options = new BuildPlayerOptions
                {
                    scenes = new[] { ScenePath },
                ...
                if (report.summary.result != UnityEditor.Build.Reporting.BuildResult.Succeeded)
                    throw new InvalidOperationException("Windows build failed: " + report.summary.result);
                Debug.Log($"[WAVE-BUILD] SUCCESS batch=20: {output} ({report.summary.totalSize:N0} bytes, {report.summary.totalTime})");
            }
            catch (Exception exception)
            {
                Debug.LogException(exception);
        ...
        public static void BuildWindows() => BuildBatch20();
